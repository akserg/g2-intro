...
export function main() {
    describe('Drag and Drop without draggable data', () => {
        ...
        it('should find all elements in steps before show the intro', () => {
            let introSteps:IntroStep[] = checkSteps(steps);

            expect(introSteps).toBeDefined();

            // Length found steps equals original
            expect(introSteps.length).toEqual(steps.length);

            // Any step item without selector or element wrapped with DIV marked with class 'introjsFloatingElement'
            expect(introSteps[0].element.className).toEqual('introjsFloatingElement');
            expect(introSteps[5].element.className).toEqual('introjsFloatingElement');
            // Any step without position must have position 'floating'
            expect(introSteps[0].position).toEqual('floating');
            expect(introSteps[5].position).toEqual('floating');

            // Specified positions must be left unchanged if step has selector or element
            expect(introSteps[2].position).toEqual('right');
            expect(introSteps[3].position).toEqual('left');
        });
    });
}
